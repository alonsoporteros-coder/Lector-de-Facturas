...
type Row = {
  id: string; file: string; destino: "Compras" | "Ventas"; ruc: string;
...
function parseInvoice(text:string,file:string): Row {
  ...
  const rucs=[...clean.matchAll(/(?:R\.?U\.?C\.?|RUC)\s*(?:N[°º.]?\s*)?:?\s*(\d{11})/gi)].map(m=>m[1]);
  const ruc=rucs[0]||valueAfter(clean,[/\b(\d{11})\b/]);
  const clienteRuc=rucs.find(x=>x!==ruc)||"";
  ...
  const missing=[ruc,fecha,compMatch?.[1],compMatch?.[2],total].filter(Boolean).length;
  return {id:crypto.randomUUID(),file,destino:"Compras",ruc,razonSocial:razon,cliente,fecha,tipo:factura?"01":nota?"NV":"",serie:compMatch?.[1]||comprobante,numero:compMatch?.[2]||"",descripcion:"Revisar detalle extraído del comprobante",base,igv,total,moneda:currency,confidence:Math.min(96,55+missing*8),status:missing>=5?"listo":"revisar"};
}
...
export default function Home(){
  const [rows,setRows]=useState<Row[]>([]); const [active,setActive]=useState<"Todos"|"Compras"|"Ventas">("Todos");
  ...
  const handleFiles=async(list:FileList|File[])=>{const files=Array.from(list).filter(f=>/\.(pdf|jpe?g|png)$/i.test(f.name));if(!files.length)return;setBusy(true);for(let i=0;i<files.length;i++){const f=files[i];try{setProgress(`Procesando ${i+1} de ${files.length}: ${f.name}`);const text=await extractText(f,s=>setProgress(`${f.name} · ${s}`));setRows(prev=>[...prev,parseInvoice(text,f.name)]);}catch{setRows(prev=>[...prev,{...initialRows[0],id:crypto.randomUUID(),file:f.name,razonSocial:"",cliente:"",ruc:"",fecha:"",serie:"",numero:"",descripcion:"No se pudo reconocer automáticamente",base:0,igv:0,total:0,confidence:0,status:"revisar"}]);}}setBusy(false);setProgress("");};
